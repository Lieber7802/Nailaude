...
This guide records the issues found while integrating and testing the AgentHub
Codex CLI adapter. It also defines the Windows and macOS development
expectations for the team.

AgentHub runs Codex as an independent one-shot subprocess. It does not reuse or
...
On Windows, AgentHub first discovers the runnable Codex Desktop cache under:
...
AgentHub starts a backend-only loopback bridge per task:
...
Every task uses a new temporary `CODEX_HOME`. AgentHub also removes desktop-only
...
AgentHub intentionally runs Codex directly on the Windows host:
...
| Absolute workspace path | Drive-letter path such as `D:\AgentHub\workspaces\demo` | POSIX path such as `/Users/name/AgentHub/workspaces/demo` | Never commit local absolute paths |
...
- A persisted or copied absolute `workDir` is machine-specific. A Windows value
  such as `D:\AgentHub\workspaces\demo` is not a usable macOS workspace, and a
